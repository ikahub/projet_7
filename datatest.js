...
class App{
    constructor(){
        this.allRecipes = document.getElementById('recipes')
    ...
    displayRecipes(json){
        json.forEach(() => {
                json.map(o => {
                    const oneRecipe = document.createElement('p')
                    oneRecipe.innerHTML = o["name"] + o["id"] + o["ingredients"]
                    this.allRecipes.appendChild(oneRecipe)
                    console.log(this.oneRecipe)
                })
        });
    }
        //const recipeName = this.recipes.find(r => r = id).id
...
function displayRecipes(){
    const dataInstance = new dataRecipes()
    const recipesDiv = document.getElementsByClassName('recipe')

    recipesDiv.innerHTML = dataInstance
}
